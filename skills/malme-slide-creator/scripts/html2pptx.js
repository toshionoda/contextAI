...
// Helper: Decode URL-encoded file paths (handles Japanese/multibyte characters)
function decodeFilePath(filePath) {
  if (!filePath) return filePath;

  // Remove file:// protocol if present
  let decodedPath = filePath.startsWith('file://') ? filePath.replace('file://', '') : filePath;

  // Decode URL-encoded characters (e.g., %E5%AD%A6 -> 学)
  try {
    decodedPath = decodeURIComponent(decodedPath);
  } catch (e) {
    // If decoding fails (e.g., already decoded), return as-is
  }
...
async function addBackground(slideData, targetSlide, tmpDir) {
  if (slideData.background.type === 'image' && slideData.background.path) {
    // Decode URL-encoded paths (handles Japanese/multibyte characters)
    const imagePath = decodeFilePath(slideData.background.path);
...
function addElements(slideData, targetSlide, pres) {
  for (const el of slideData.elements) {
    if (el.type === 'image') {
      // Decode URL-encoded paths (handles Japanese/multibyte characters)
      const imagePath = decodeFilePath(el.src);
      ...
      // Build table data array for PptxGenJS
      const tableData = el.rows.map(row =>
        row.map(cell => {
          // Handle both simple text and formatted text runs
          if (typeof cell.text === 'string') {
            return {
              text: cell.text,
              options: {
                ...(cell.options.fill && { fill: cell.options.fill }),
                color: cell.options.color,
                bold: cell.options.bold,
                italic: cell.options.italic,
                fontSize: cell.options.fontSize,
                fontFace: cell.options.fontFace,
                align: cell.options.align,
                valign: cell.options.valign,
                ...(cell.options.colspan && { colspan: cell.options.colspan }),
                ...(cell.options.rowspan && { rowspan: cell.options.rowspan }),
                ...(cell.options.border && { border: cell.options.border })
              }
            };
          } else {
            // Formatted text runs
            return {
              text: cell.text,  // Array of text runs
              options: {
                ...(cell.options.fill && { fill: cell.options.fill }),
                ...(cell.options.colspan && { colspan: cell.options.colspan }),
                ...(cell.options.rowspan && { rowspan: cell.options.rowspan }),
...
async function extractSlideData(page) {
  return await page.evaluate(() => {
    ...
    // Helper: Parse table element into structured data
    const parseTable = (tableEl) => {
      const tableRect = tableEl.getBoundingClientRect();
      const tableComputed = window.getComputedStyle(tableEl);
      ...
      rowElements.forEach((tr, rowIndex) => {
        ...
        rowHeights.push(pxToInch(rowRect.height));
        ...
        cellElements.forEach((cell, colIndex) => {
          ...
          // Track column widths from first row
          if (rowIndex === 0) {
            colWidths.push(pxToInch(cellRect.width));
          }

          const isHeader = cell.tagName === 'TH';
          const bgColor = cellComputed.backgroundColor;
          ...
          // Extract per-cell borders (all 4 sides: top, right, bottom, left)
          const cellBorders = ['Top', 'Right', 'Bottom', 'Left'].map(side => {
            const w = parseFloat(cellComputed[`border${side}Width`]) || 0;
            return w > 0
              ? { pt: w * PT_PER_PX, color: rgbToHex(cellComputed[`border${side}Color`]) }
              : { pt: 0, color: 'FFFFFF' };
          });
          ...
          cells.push({
            text: cellText,
            options: {
              fill: hasBg ? { color: rgbToHex(bgColor) } : null,
              ...
              valign: cellComputed.verticalAlign === 'middle' ? 'middle' : (cellComputed.verticalAlign === 'bottom' ? 'bottom' : 'top'),
      ...
      return {
        type: 'table',
        position: {
          x: pxToInch(tableRect.left),
          y: pxToInch(tableRect.top),
    ...
    document.querySelectorAll('*').forEach((el) => {
      ...
      if (el.tagName === 'TABLE') {
        ...
        if (tableData.rows.length > 0) {
          ...
          // Mark all table descendant elements as processed (including text elements inside cells)
...
async function html2pptx(htmlFile, pres, options = {}) {
  ...
  try {
    ...
    // Spread process.env to preserve HOME, PATH, etc. (replacing env loses them)
    const launchOptions = { env: { ...process.env, TMPDIR: tmpDir } };
    if (process.platform === 'darwin') {
      launchOptions.channel = 'chrome';
    } else {
      // Container/sandbox environments (e.g., Claude browser) require these flags
      // --single-process: fixes page.goto crash with file:// URLs in containers
      launchOptions.args = [
        '--no-sandbox',
